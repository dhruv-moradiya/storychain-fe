...
import { CommentTree } from '@/components/stories/panels/comments/CommentTree';
...
export default function StoryRequestDetail({ request, onBack }: StoryRequestDetailProps) {
    ...
    <div className="flex h-full w-full flex-col p-4">
      ...
      <div className="flex items-center justify-between border-b pb-4">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" className="size-7" onClick={onBack}>
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <div className="flex justify-between gap-4">
          <div className="flex flex-col gap-1">
            <span className="text-muted-foreground text-xs">Chapter</span>
        ...
        <div>
          ...
          >
            ...
            {request.changeType === 'new' ? 'New Chapter' : 'Update Existing Chapter'}
          </Button>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle className="text-base font-semibold">
                  {request.changeType === 'new' ? 'New Chapter Content' : 'Changes in Chapter'}
                </DialogTitle>
              </DialogHeader>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {request.contributors.map((contrib) => (
                  <Badge key={contrib} variant="outline" className="text-xs font-medium">
                    {contrib}
                  </Badge>
                ))}
              </div>
        ...
        <div className="flex items-center gap-2">
          <Avatar className="size-7">
            {request.author.avatar && (
              <AvatarImage src={request.author.avatar} alt={request.author.name} />
            )}
            <AvatarFallback className="text-xs">{request.author.name.slice(0, 2)}</AvatarFallback>
          </Avatar>
        ...
        <div>
          ...
          <Textarea placeholder="Write your feedback..." className="h-20 resize-none text-sm" />
        </div>
      </div>

      {/* Footer Actions */}
      <div className="bg-background mt-auto flex justify-end gap-2 border-t p-3">
        ...
        {request.status === 'rejected' && (
          <Button variant="outline" size="sm" className="flex h-8 items-center gap-1.5 text-xs">
        ...
        {request.status === 'merged' && (
          <span className="text-muted-foreground flex items-center gap-1.5 text-xs">
            <Check className="size-3.5" /> This chapter request has been accepted
          </span>
        )}
      </div>
    </div>
