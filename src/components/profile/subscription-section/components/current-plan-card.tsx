...
import { Crown, Zap, BookOpen, ArrowRight, Calendar, RefreshCw, Pause, Play } from 'lucide-react';
import { Link } from 'react-router';
import type { UserSubscription, PlanType } from '../subscription.types';
import { planDetails } from '../subscription.data';
    ...
    <div className="border-border/50 bg-cream-95 rounded-xl border p-5">
      <div className="mb-4 flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div
            className={cn('flex h-12 w-12 items-center justify-center rounded-xl', plan.bgColor)}
          >
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {plan.features.slice(0, 4).map((feature) => (
          <span
            key={feature}
            className="border-border/50 text-text-secondary-65 rounded-full border bg-white/50 px-2.5 py-1 text-xs"
          >
            {feature}
          </span>
        ))}
        ...
        {isFreePlan ? (
          ...
          <>
            ...
            {subscription?.status === 'ACTIVE' ? (
              <Button variant="outline" className="gap-2 text-amber-600 hover:text-amber-700">
